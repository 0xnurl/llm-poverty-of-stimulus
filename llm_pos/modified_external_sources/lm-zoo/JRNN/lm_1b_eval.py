...
tf.flags.DEFINE_integer(
    "num_samples", 3, "Number of samples to generate for the prefix."
)
...
tf.flags.DEFINE_integer(
    "max_eval_steps", 1000000, 'Maximum mumber of steps to run "eval" mode.'
)
    ...
    for _ in xrange(FLAGS.num_samples):
        ...
        while True:
            ...
            softmax = sess.run(
                t["softmax_out"],
                feed_dict={
                    t["char_inputs_in"]: char_ids_inputs,
                    t["inputs_in"]: inputs,
                    t["targets_in"]: targets,
                    t["target_weights_in"]: weights,
                },
...
def _DumpEmb(vocab):
    ...
    for i in xrange(vocab.size):
        ...
        if "char_inputs_in" in t:
            input_dict[t["char_inputs_in"]] = vocab.word_char_ids[i].reshape(
                [-1, 1, MAX_WORD_LEN]
            )

...
SURPRISAL_RE = re.compile(
    r"sentence_id\ttoken_id\ttoken\tsurprisal\n"
    "(\d+\s+\d+\s+[\w.<>|\u2581]+\s+[-\d.]+\n)+(\d+\s+\d+\s+[\w.<>|\u2581]+\s+[-\d.]+)",
    flags=re.MULTILINE,
)
...
class LMProcessingTest(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        cls._predictions_supported = cls.spec["image"]["supported_features"][
            "get_predictions"
        ]
        ...
        with text_f, predictions_f:
            ...
            cls.tokenized_output = subprocess.check_output(
                ["tokenize", text_f.name]
            ).decode("utf-8")
            print(cls.tokenized_output)
            print("\n")

            print("== unkify %s" % text_f.name)
            cls.unkified_output = subprocess.check_output(
                ["unkify", text_f.name]
            ).decode("utf-8")
            ...
            cls.surprisals_output = subprocess.check_output(
                ["get_surprisals", text_f.name]
            ).decode("utf-8")
            ...
            if cls._predictions_supported:
                ...
                cls.predictions_output = subprocess.check_output(
                    ["get_predictions.hdf5", text_f.name, predictions_f.name]
                ).decode("utf-8")
                print(cls.predictions_output)
                cls.predictions_data = h5py.File(predictions_f.name, "r")

        cls.tokenized_lines = [
            line.strip() for line in cls.tokenized_output.strip().split("\n")
        ]
        cls.unkified_lines = [
            line.strip() for line in cls.unkified_output.strip().split("\n")
        ]
        cls.surprisal_lines = [
            line.strip().split("\t")
            for line in cls.surprisals_output.strip().split("\n")
        ]
        ...
        for tokenized_line in self.tokenized_lines:
            for token in tokenized_line.split(" "):
                assert token in self.spec["vocabulary"]["items"], (
                    "%s missing from model vocabulary spec, but output by tokenize"
                    % token
                )
    ...
    def test_tokenization_match_surprisals(self):
        ...
        for i, tokenized_line in enumerate(self.tokenized_lines):
            ...
            eq_(
                tokens,
                [surprisals[i + 1][j + 1][0] for j in range(len(tokens))],
                "Token sequences should match exactly",
            )
    ...
    def test_surprisal_parse(self):
        eq_(
            set(int(line[0]) for line in self.surprisal_lines[1:]),
            {1, 2, 3, 4},
            "Sentences retained",
        )
    ...
    def test_tokenization_match_predictions(self):
        ...
        eq_(
            len(self.predictions_data["/sentence"]),
            len(self.tokenized_lines),

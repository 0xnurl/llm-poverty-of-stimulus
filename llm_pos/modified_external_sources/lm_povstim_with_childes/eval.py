...
from utils import batchify, batchify_finetuning, get_batch, repackage_hidden

parser = argparse.ArgumentParser(description="Evaludate finetuned model")
parser.add_argument(
    "--data", type=str, required=True, help="path to data to evaluate on"
)
parser.add_argument(
    "--model", type=str, default="model.pt", help="path to model to evaluate"
)
parser.add_argument(
    "--results", type=str, default="results.txt", help="path to save results"
)
parser.add_argument("--log", type=str, default="log.txt", help="path to logging file")
parser.add_argument(
    "--finetuning_data", type=str, required=True, help="path to finetuning data"
)
parser.add_argument("--rnn", action="store_true", help="evaluating a RNN")
parser.add_argument("--seed", type=int, default=999, help="random seed")
parser.add_argument("--batch_size", type=int, default=500, help="batch size")
parser.add_argument("--log_interval", type=int, default=10, help="log interval")
parser.add_argument("--cuda", action="store_true", help="use CUDA")
parser.add_argument("--print", action="store_true", help="print sentences and results")
parser.add_argument("--ppl", action="store_true", help="get sentence ppls")
parser.add_argument("--recall", type=int, default=0, help="get recall@k")
parser.add_argument("--babyberta", action="store_true", help="evaluate babyberta")
...
def evaluate_ppl(data_source):
    ...
    with torch.no_grad():
        for batch, b in enumerate(data_source):
            ...
            ppls += [
                torch.exp(criterion(o, t)).item()
                for o, t in zip(output.permute(1, 0, 2), targets.T)
            ]
...
def evaluate_recall(data_source):
    ...
    with torch.no_grad():
        for batch, i in enumerate(range(0, data_source.size(0) - 1, args.batch_size)):
            ...
            preds += (
                torch.topk(output, k=args.recall, dim=2)[1]
                .permute(1, 0, 2)
                .reshape(-1, args.recall)
                .tolist()
            )
    return preds
...
def accuracy(preds, targets):
    with torch.no_grad():
        ...
        for pred, target in zip(preds, targets):
            pred_aux = idx2word[
                pred[target.index(corpus.dictionary.word2idx["."])]
            ]  # note that the predictions are shifted by one compared to the targets (since there is no prediction for the first word), which is why we don't add one to the index here
...
if not args.recall:
    eval_data_flat = [
        x[: x.index(0)] if 0 in x else x for y in eval_data for x in y.tolist()
    ]
if args.babyberta:
    ...
    with open(args.results, "w") as fo:
        ...
        for r, d, f, m, p in zip(
            results, eval_data_flat, move_first, move_main, pred_auxs
        ):
            fo.write(
                "target:\t"
                + str(f)
                ...
                + str(m + 2)
                + "\t"
                + " ".join([corpus.dictionary.idx2word[word] for word in d])
                + "\n"
            )
            fo.write(
                "predicted:\t"
                ...
                + str(m + 2)
                + "\t"
                + " ".join([corpus.dictionary.idx2word[word] for word in r])
                + " "
                + p
                + " "
                + "\n\n"
    ...
    # logging.info('=' * 89)
    logging.info(
        "\nProportion that the first word follows MOVE-FIRST: "
        + (str(sum(move_first) / len(move_first)) + "0" * 6)[:6]
        + "\nProportion that the first word follows MOVE-MAIN: "
        + (str(sum(move_main) / len(move_main)) + "0" * 6)[:6]

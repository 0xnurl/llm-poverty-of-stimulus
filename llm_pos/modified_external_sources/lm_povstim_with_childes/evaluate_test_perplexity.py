...
parser = argparse.ArgumentParser(
    description="Evaluate perplexity of the dataset, ignoring the <unk> words"
)
parser.add_argument(
    ...
    with torch.no_grad():
        ...
        for i in range(0, data_source.size(0) - 1, args.bptt):
            ...
            output_flat = output_flat[
                torch.arange(0, output_flat.size(0), out=out_type)[subset]
            ]

            total_len += targets.size(0)
            total_loss += (
                targets.size(0) * nn.CrossEntropyLoss()(output_flat, targets).item()
            )
